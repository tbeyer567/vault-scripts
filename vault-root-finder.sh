...
# Finds Vault token accessors that have the "root" policy or the 'hcp-root' policy.
# Requires:
...
echo "$accessors_json" | jq -r '.[]?' | while read -r accessor; do
  ...
  has_root="$(echo "$lookup_json" | jq -e '.data.policies | (index("root") or index("hcp-root"))' 2>/dev/null && echo "yes" || echo "")"
  if [[ -z "$has_root" ]]; then
    continue
  fi
